...
if st.button("Predict"):
   ...
   # 【核心修复】根据返回数据的维度动态处理，避免索引错误
   # 情况 A: 返回的是列表 (旧版 SHAP 多分类/二分类) -> [class_0_vals, class_1_vals]
   if isinstance(shap_values_raw, list):
      shap_val_0 = shap_values_raw[0][0]
      shap_val_1 = shap_values_raw[1][0]
      base_0 = expected_value[0]
      base_1 = expected_value[1]

   # 情况 B: 返回的是 3维数组 (N, Features, Classes)
   elif len(shap_values_raw.shape) == 3:
      shap_val_0 = shap_values_raw[0, 0, :]
      shap_val_1 = shap_values_raw[0, 1, :]
      base_0 = expected_value[0]
      base_1 = expected_value[1]

   # 情况 C: 返回的是 2维数组 (N, Features) -> 通常是二分类中类别 1 的值
   else:
      shap_val_1 = shap_values_raw[0]  # 直接取第一行作为类别 1 的值
      shap_val_0 = -shap_val_1  # 二分类中，类别0的SHAP值约为类别1的相反数

      # 处理期望值：如果是标量，则 base_1 = val, base_0 = -val
      if np.isscalar(expected_value):
         base_1 = expected_value
         base_0 = -expected_value
      else:
         # 如果 expected_value 是数组但 shap 是 2 维，取对应值
         base_1 = expected_value[1] if len(expected_value) > 1 else expected_value[0]
         base_0 = -base_1
   ...
   try:
      ...
      if predicted_class == 1:
         # 绘制类别 1 (HAD)
         shap.plots.force(base_1, shap_val_1, input_df.iloc[0], matplotlib=True)
      else:
         # 绘制类别 0 (No HAD)
         shap.plots.force(base_0, shap_val_0, input_df.iloc[0], matplotlib=True)
      ...
      st.error(f"SHAP 绘图失败：{e}")
      st.info("请检查模型文件或 SHAP 库版本兼容性。")
